#!/usr/bin/env bash
# Get a realm ready for mailaas: run with the CLI pointed at the realm, before
# it is installed.
#
# Usage: prepare-realm.sh
#
# - Overbook the realm's hypervisor.  A warm realm is sized for the pool's
#   defaults, and the metapaas CP plus the mail data plane do not both fit on it
#   one to one -- the runner test overbooked the same way.
# - Install metapaas, which mailaas plugs into, unless the realm already has it,
