#!/usr/bin/env bash
# Run the functional suite on the realm, where the mail data plane is.
#
# Usage: run-functional-in-realm.sh
#
# The suite talks SMTP to the mail instance on its own address, which only the
...
# An ACTIVE mailaas element only means the core finished reconciling it: the
# metapaas control plane it registers the mail type in is a VM that may still be
...
while :; do
    code="$(curl -s -m 10 -o /dev/null -w '%{http_code}' \
        "$cp_url/v1/types/mail/versions/")" || code=000
...
export EXORDOS_MAIL_CP_URL=http://localhost/api/metapaas
export EXORDOS_USERNAME=admin
EXORDOS_PASSWORD="$(cat ~/.mail_admin_password)"
